...
- [x] **V2-019: Formalize token budgeting and counting.** Added one resolved provider/model budget shared by T15/T16, explicit limit precedence and reserves, mandatory-first below-target behavior, pinned tokenizer identity/checksum, validated `utf8_bytes_v1` fallback, pre-send recounting, revision/packet identity inputs and Unicode/JSON determinism tests across T15, T16, requirements, architecture and `LLD.md` section 29.1. Review refs: Ultra A8; Findings H6; Fable F13.
- [x] **V2-020: Complete provider retry accounting.** Added a persisted per-pass call ledger, configurable three-call default, one shared transport retry, one mutually exclusive fallback-or-repair slot, deterministic precedence, atomic reservation, per-call metadata, crash-safe counters and terminal reasons in T16 plus LLD/requirements/architecture configuration. Review refs: Ultra A20/M5; Findings M5.
- [x] **V2-021: Implement the central policy/profile/dictionary resolver.** Expanded `LLD.md` section 29 with allowlisted lookup, schema/checksum/compatibility validation, fatal missing/corrupt behavior, immutable policy-set revision and profile/model resolution; replaced bare version strings in T04, T11-T14 and T21-T23 with resolved handles and added the startup reliability requirement. Review refs: Ultra A20/M4; Findings M4.
- [x] **V2-022: Make dependency expansion accounting shared.** Defined one T24-owned `ExpansionBudget`/`ExpansionDecision` ledger shared by T22 and T24, atomic single-counter consumption, deterministic proposals/clamps/denials, original/requested/effective bounds, reason/evidence and revision/persistence/tests. Review refs: Ultra A20/M11; Findings M11.
- [x] **V2-023: Convert T23 impact semantics into an executable decision table.** Added ordered eligibility, requirement mapping, causal-link, temporal, recovery/bypass, contradiction, earliest-cause and counterfactual gates; explicit terminal precedence; replayable decision traces; and causal-versus-contributing tests in T23 section 8.1. Review refs: Ultra A20/M10; Findings M10.
- [x] **V2-024: Make `REGISTRATION COMPLETE` conditional.** Made initial/mobility/periodic completion depend on the release/deployment acknowledgement rule derived from Registration Accept, with true/false/unknown semantics, allowlisted fact, profile YAML contract and required/missing/not-required/invisible fixtures across requirements, LLD, profiles and T09. Review refs: Ultra U8.
- [x] **V2-025: Add a roaming topology and fault-domain producer.** Assigned T03 typed time-bounded topology and independent domain-map production from PLMN/SUCI/GUAMI/TAI/NF-domain/path evidence, with alternatives, scores, confidence, revisions/indexes and explicit consumption rules for T04/T11/T12/T14/T17. Review refs: Ultra A20/F2; Fable F2.
- [x] **V2-026: Complete concrete non-3GPP contracts.** Added gNB/N3IWF/TNGF `AccessContextKey` and access-scoped registration state/indexes in T03, trusted/untrusted profile anchors and fixtures, T04 access fields/state/non-merge/transfer/deregistration rules, and aligned LLD/requirements for concurrent 3GPP/non-3GPP histories. Review refs: Ultra A20/F5; Fable F5.
- [x] **V2-027: Make interface visibility release/profile-aware.** Replaced the flat visibility enum with registry-backed reference-point, SBI service and SBI API namespaces; added release/profile definitions for N7, N13, N35, N36 and N37; kept `Nnrf` in SBI service visibility and `N27` for NRF-to-NRF roaming; aligned requirements, architecture, LLD, profile registry, T04/T09/T14 and detector context wording. Review refs: Ultra A18.
