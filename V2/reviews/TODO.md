...
- [ ] **V2-003: Define attempt selection and model-call policy.** Support explicit attempt/UE selection plus deterministic configurable ordering and caps for model narration. Reports must disclose attempts analyzed deterministically but skipped by the model. Review refs: Ultra A4.
- [ ] **V2-004: Separate evidence stage from model pass.** Define one `EvidenceStage` for T12/T15, one `ModelPass` for T16, legal transitions, and the mapping between them. Do not collapse both concepts into one enum. Review refs: Ultra A10.
- [ ] **V2-005: Canonicalize dependency request and result contracts.** Define `DependencyReasonCode`, the generic T16 request, adapters to typed T24/T25 requests, and the discriminated `DependencyInspectionResult`. Keep T23's dependency discriminator; do not add redundant type fields to T24/T25. Review refs: Ultra C2/A9; Findings M6.
- [ ] **V2-006: Define immutable `FailureCandidate` ownership.** Specify owners/defaults for severity, relevance, capture phase, call impact, detector score, and named detector score terms. Primary candidates default to inconclusive dependency impact; T23 produces dependency impact; T12 ranks without mutating source observations. Review refs: Ultra A5/R20; Findings H1/L3.
- [ ] **V2-007: Add a shared `DetectionContext`.** Supply capture bounds, T21 phase access, interface visibility, correlation confidence, and resolved profile/policy handles to T06-T09. Review refs: Ultra A5; Findings M3.
- [ ] **V2-008: Implement the central evidence registry contract.** Define deterministic evidence identity, `EvidenceRecord`, normalized source references, semantic record types, revision scope, collision handling, deduplication, storage/index creation, and T18 resolution. Evidence must resolve without T15 or a model provider. Review refs: Ultra A2; Fable F7.
- [ ] **V2-009: Publish the shared-model registry.** Give every cross-tool model one owner/module, schema version, serialization schema, and import direction. Include artifacts, collections, visibility, capture metadata, matchers/conditions, warnings, evidence capabilities, report state/policy, and dependency unions. Compare `DependencyBaselineComparison` and `UDRBaselineComparison` field-by-field; consolidate them when semantically identical, otherwise define a common base plus an explicit UDR specialization. Review refs: Ultra A9/A11; Findings L9.
- [ ] **V2-010: Define all T13 nested schemas.** Specify selectors, expected requests, time scopes, ordering, extracted spans, conflicts, matchers, conditions, validation limits, and local/remote masking modes. Review refs: Findings L8.
- [ ] **V2-011: Define revision envelopes and immutable publication.** Specify canonical serialization and algorithm-prefixed digest, parent revisions, input checksums, tool/config/policy/schema versions, compatibility, and manifest-last publication. Every artifact producer exposes its output revision; consumers record the exact input revisions used. Review refs: Ultra A14; Fable F10.
- [ ] **V2-012: Define the machine-readable issue registry.** Add stable namespaced codes with severity, stage, message template, evidence references, retryability, and aggregation behavior. Prohibit unregistered ad hoc machine codes. Review refs: Ultra A12; Fable F8.
- [ ] **V2-013: Define the run manifest and `AnalysisState`.** Record stage status/revision/artifacts/issues, selected attempts, provider/dependency executions, and publication outcome. Aggregate domain-specific statuses according to stage criticality and policy rather than one universal status enum. Review refs: Ultra A16; Fable F12.
- [ ] **V2-014: Canonicalize root-cause scoring.** Reconcile LLD and T12 terms, meanings, defaults, bounds, scalar derivation, named score-term persistence, deterministic ties, and revision inputs. Review refs: Ultra A3; Findings H2/L3.
- [ ] **V2-015: Canonicalize baseline selection.** Choose and document one T11/LLD algorithm. Prefer lexicographic eligibility and similarity ordering, retaining numeric scores for audit without overriding priority. Define deterministic tie-breaking. Review refs: Ultra A3; Findings H3.
- [ ] **V2-016: Reconcile T03 identity-link thresholds.** Define named T03 bands (`automatic >= 0.90`, warning-link `0.70-0.89`, candidate `<0.70`), validated config fields, conflict handling, and threshold-boundary tests. Remove or rename the contradictory `minimum_auto_link_confidence=0.75`. Review refs: Ultra A3; Findings M1.
- [ ] **V2-061: Clarify T07/T09 missing-transition ownership.** Keep T09 as the sole implicit missing-transition detector with its `0.65` base. T07 owns explicit NAS/NGAP protocol/state failures and may preserve request-only observations for T09, but must not emit duplicate missing-response candidates. Align both scope sections, score tables, and tests. Review refs: Ultra A3; Findings M2.
- [ ] **V2-017: Complete the procedure-profile registry and traceability.** Keep implementation ownership under T04/profile subsystem and place the single normative registry contract at `V2/profiles/README.md`, referenced by T04, T09, LLD, and `tools/README.md`. Define file/schema format, API, release/deployment overlays, conditional grammar, ordering, version/checksum, compatibility, authoring/review process, and mappings from every requirement section-8 flow/acceptance criterion to profile IDs and fixtures. Review refs: Ultra A1; Fable F1.

## P1: Runtime, Protocol, and Scenario Behavior

- [ ] **V2-018: Define honest targeted re-decode bounds.** Distinguish result-size, decoder-resource, and source-scan bounds. Preserve TCP/HTTP2/HPACK, SCTP, and fragmentation context. Build a packet offset/time/frame index during T01 if source-size-independent access is required; otherwise document and benchmark O(source-position) scanning. Retain complete source/slice provenance and safe staging cleanup. Review refs: Ultra A7; Findings H5.
